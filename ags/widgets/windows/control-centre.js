...
import { NetworkIp, NetworkWifiToggle, NetworkWifiSelection } from '../network.js';
import {
    AudioSpeakerToggle,
...
const { control_centre, theme, placeholders } = Options;
...
const Filler = () =>
    Widget.Label({
        ...
        setup: (self) => {
            const update = () => {
                const size = Object.keys(placeholders).length;
                const num = Math.floor(Math.random() * size);
                self.label = placeholders[num];
            };
    ...
    Popup({
        ...
        child: Column({
            children: [
                    ...
                    children: [
                        Row({
                            ...
                            setup: (self) =>
                                self.hook(
                                    opened.menu,
                                    (self) => (self.spacing = opened.menu.value != '' ? 15 : 0)
                                ),
                        }),
                        Row({
                            children: [
                        ...
                        Column({
                            children: [
                                CalendarBox(),
                                Row({
                                    children: [MediaButton({ name: 'media' }), Filler()],
                                    spacing: 15,
                                }),
                            ],
                            spacing: 15,
                ...
                Row({ children: [Padding(), MediaBox({ name: 'media' }), Padding()] }),
            ],
        }),
    });
